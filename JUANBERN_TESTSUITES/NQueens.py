...
# Pure-Python implementation of itertools.permutations().
def permutations4(iterable, r=None):
    """permutations(range(3), 2) --> (0,1) (0,2) (1,0) (1,2) (2,0) (2,1)"""
    pool = tuple(iterable)
    n = len(pool)
    if r is None:
...
def n_queens4(queen_count):
    ...
    Yields:
        ...
        queen, and the index into the tuple indicates the row.
    """
    cols = range(queen_count+1) # <-- FIX cols = range(queen_count)
    for vec in permutations2(cols):
        if (queen_count == len(set(vec[i] + i for i in cols))
                        == len(set(vec[i] - i for i in cols))):
            yield vec

def n_queens5(queen_count):
    """N-Queens solver.
...
def aBench_n_queens4(queen_count):
    return list(n_queens4(queen_count))

def aBench_n_queens5(queen_count):
    return list(n_queens5(queen_count))
